...
def concat_all_data(directory):
    ...
    for each_file in files:
        if "xlsx" or "xls" in each_file:
            df = polars.read_excel(each_file)
            df.with_column("File_Name", each_file)
            df.with_column("File_Name", df.index + 2)
            list_of_dfs.append(df)
        elif "csv" in each_file:
            df = polars.read_csv(each_file)
            df.with_column("File_Name", each_file)
            df.with_column("File_Name", df.index + 2)
            list_of_dfs.append(df)
        else:
